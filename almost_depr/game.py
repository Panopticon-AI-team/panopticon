"""
This is the old game code. Also too simplistic
It won't allow you to have states and actions, so we can't use it
"""
...
class MovingAssets:
    ...
    def init_units(self):
        ...
        for i in range(red_ct):
            red_force_arr[i] = Asset(
                self.canvas, 150 + i, 150 + i, color="red", speedmult=0.8
            )

        for i in range(blue_ct):
            blue_force_arr[i] = Asset(
                self.canvas, 250 + i, 150 + i, color="blue", speedmult=1.4
            )
