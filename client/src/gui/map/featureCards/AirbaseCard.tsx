...
export default function AirbaseCard(props: Readonly<AirbaseCardProps>) {
    ...
    const [aircraftCount, setAircraftCount] = useState(props.airbase.aircraft.length);
    ...
    const airbaseDataContent = (
        <>
            <Typography variant="h5" component="div">{props.airbase.name}</Typography>
            <Typography variant="h6">Coordinates: {props.airbase.latitude.toFixed(2)}, {props.airbase.longitude.toFixed(2)}</Typography>
            <Typography variant="h6">Altitude: {props.airbase.altitude.toFixed(2)} FT</Typography>
        ...
        return (
            <form>
                <Stack spacing={1} direction="column">
                    <Typography variant="h5" component="div">EDIT AIRBASE</Typography>
                    <TextField autoComplete='off' id="airbase-name-text-field" label="Name" defaultValue={props.airbase.name} onChange={_handleTextFieldChange} variant="outlined" sx={inputStyle} InputLabelProps={inputLabelStyle}/>
                </Stack>
            </form>
        )
    }
    ...
    const editingCardActions = (
        <Stack spacing={1} direction="column">
            <Stack spacing={1} direction="row">
                <Button variant="contained" size="small" onClick={handleSaveEditedAirbase} startIcon={<SaveIcon/>}>Save</Button>
    ...
    return (
        <FeaturePopup anchorPositionTop={props.anchorPositionTop} anchorPositionLeft={props.anchorPositionLeft} content={airbaseCard} handleCloseOnMap={props.handleCloseOnMap}></FeaturePopup>
    );
}