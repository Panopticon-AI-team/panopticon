...
export default function MultipleFeatureSelector({ features, handleSelectSingleFeature, handleCloseOnMap, anchorPositionTop, anchorPositionLeft }: Readonly<MultipleFeatureSelectorProps>) {
    const _handleSelectFeature = (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const selectedFeature = features.find(feature => feature.getProperties()?.id === selectedFeatureId)
    ...
    return (
        <FeaturePopup anchorPositionTop={anchorPositionTop} anchorPositionLeft={anchorPositionLeft} content={multipleFeatureSelector} handleCloseOnMap={handleCloseOnMap}></FeaturePopup>
    );
}