...
export default class Game {
  ...
  createScenarioCopy(scenario: Scenario): Scenario {
    const savedSides = scenario.sides.map((side: Side) => {
      const newSide = new Side({
        id: side.id,
        name: side.name,
        totalScore: side.totalScore,
    ...
    const scenarioCopy = new Scenario({
      id: scenario.id,
      name: scenario.name,
      ...
      sides: savedSides,
      timeCompression: scenario.timeCompression,
    });
    scenario.aircraft.forEach((aircraft: Aircraft) => {
      const newAircraft = new Aircraft({
        id: aircraft.id,
        name: aircraft.name,
        sideName: aircraft.sideName,
      ...
      scenarioCopy.aircraft.push(newAircraft);
    });
    scenario.airbases.forEach((airbase: Airbase) => {
      const airbaseAircraft: Aircraft[] = [];
      airbase.aircraft.forEach((aircraft: Aircraft) => {
        const newAircraft = new Aircraft({
          id: aircraft.id,
      ...
      scenarioCopy.airbases.push(newAirbase);
    });
    scenario.facilities.forEach((facility: Facility) => {
      const newFacility = new Facility({
        id: facility.id,
        name: facility.name,
        sideName: facility.sideName,
      ...
      scenarioCopy.facilities.push(newFacility);
    });
    scenario.weapons.forEach((weapon: Weapon) => {
      const newWeapon = new Weapon({
        id: weapon.id,
        name: weapon.name,
        sideName: weapon.sideName,
      ...
      scenarioCopy.weapons.push(newWeapon);
    });
    scenario.ships?.forEach((ship: Ship) => {
      const shipAircraft: Aircraft[] = [];
      ship.aircraft.forEach((aircraft: Aircraft) => {
        const newAircraft = new Aircraft({
          id: aircraft.id,
      ...
      scenarioCopy.ships.push(newShip);
    });
    scenario.referencePoints?.forEach((referencePoint: ReferencePoint) => {
      const newReferencePoint = new ReferencePoint({
        id: referencePoint.id,
        name: referencePoint.name,
        sideName: referencePoint.sideName,
      ...
      scenarioCopy.referencePoints.push(newReferencePoint);
    });
    scenario.missions?.forEach((mission: Mission) => {
      const baseProps = {
        id: mission.id,
        name: mission.name,
        sideId: mission.sideId,
